...
for path in glob("healthy/*.jpg"):
    ...
    img_gammaed = adjust_gamma(img_equalized)
    # green = image[:, :, 1]
    ...
    ax[1].imshow(frangi(equalize_adapthist(imggreen)), cmap=plt.cm.gray)
    ax[1].set_title('Equalized')

    ax[2].imshow(frangi(equalize_adapthist(img)), cmap=plt.cm.gray)
    ax[2].set_title('Gammaed')
